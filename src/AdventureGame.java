...
/**
 * GUI class to drive the game and display it to the user. 
 */
		...
		if (gameBoard.ok(c.getRow()+dr,c.getCol()+dc)) {
			...
			// Try and make the move.
				...
				if (cw.modifyCave(newC)) {
					updateGameBoard();
					updateStatus(ch.getName()+" has successfully moved and "+
					...
					if (ch instanceof Adventurer) {
						if (!treasureClaimed) {
							JOptionPane.showMessageDialog(frame, ch.getName()+" has died! " +
									"There is no way to get the treasure now. Better luck next time!");
						}
						else {
					...
					updateStatus(ch.getName()+" has successfully moved!");
					updateGameBoard();
				}
				...
				updateStatus(ch.getName()+" is not a miner so it can't clear blocked caves!");
			}
			else {
